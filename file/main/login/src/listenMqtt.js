...
function listenMqtt(defaultFuncs, api, ctx, globalCallback) {
  ...
  mqttClient.on("connect", function () {
    if (process.env.OnStatus == undefined) {
      if (require(process.cwd() + "/main/json/config.json").restartMqtt.enable || require(process.cwd() + "/main/json/config.json").restartMqtt.minutes > 0) {
        setTimeout(() => {
          logger.load("Closing MQTT Client...");
          ctx.mqttClient.end();
          logger.load("Reconnecting MQTT Client...");
          getSeqID();
        }, Number(require(process.cwd() + "/main/json/config.json").restartMqtt.minutes) * 60 * 1000);
      } 
    }
...
function parseDelta(defaultFuncs, api, ctx, globalCallback, v) {
  if (v.delta.class === "NewMessage") {
    ...
    (function resolveAttachmentUrl(i) {
      if (!v.delta.attachments || !Array.isArray(v.delta.attachments) || i >= v.delta.attachments.length) {
        var fmtMsg;
        try {
