...
let fs = require('fs');
let folder = './system/data/banking_accounts/'; if (!fs.existsSync(folder))fs.mkdirSync(folder);
...
exports.onRun = async o=> {
    ...
    switch (o.args[0]) {
        ...
        case 'register': {
                ...
                send(`🆕 Bạn muốn tự đặt stk theo ý mình không? Reply tin nhắn này bằng stk muốn đặt (phí ${phí.toLocaleString()}$) hoặc 'n' để bỏ qua`).then(res=>(res.name = exports.config.name, res.callback = async o=> {
                    ...
                    send(`📌 Bạn muốn tự đặt mật khẩu tài khoản hay hệ thống random mật khẩu\n\nPhản hồi 'y' để tự đặt hoặc 'n' để hệ thống random`, o).then(res=>(res.name = exports.config.name, res.callback = async o=> {
                        let call = {
                            y: _=>send('✅ Hệ thống đã gửi bước nhập mật khẩu trong tin nhắn riêng', o).then(()=>send('📌 Reply tin nhắn này để điền mật khẩu bạn muốn đặt', o.event.senderID).then(res=>(res.name = exports.config.name, res.callback = o=>create_account(o.event.args[0]), res.o = o, global.Seiko.onReply.push(res)))),
                ...
                }, res.o = o, global.Seiko.onReply.push(res)));
            };
            break;
        case '-i':
        case 'info': try {
        ...
        case 'delete': {
            ...
            if (BigInt(data.debt) > 0n)return send('M thích trốn nợ khongg?😡');
            let callback = ()=> {
                del(sid);
