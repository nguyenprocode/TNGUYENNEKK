...
const API_KEY = "AIzaSyDK-uoGR0KICzXs-h_GPSpwGWcRWeudzyA";
...
function saveUrls(uid, urls) {
  const urlsFile = `system/data/uids/${uid}_urls.json`;
...
function loadChatHistory(uid) {
  const chatHistoryFile = `system/data/uids/${uid}.json`;
...
function appendToChatHistory(uid, chatHistory) {
  ...
  try {
    if (!fs.existsSync("system/data/uids")) {
...
function clearChatHistory(uid) {
  const chatHistoryFile = `system/data/uids/${uid}.json`;
  const urlsFile = `core/ids/${uid}_urls.json`;
...
module.exports = {
  config: {
    ...
    aliases: ["ai", "ggai"],
    role: 0,
    author: "Shikaki - NDK-[FIX and Cover]", // conver by LocDev
    info: "trò chuyện cùng gemini 1.5 pro",
    Category: "AI",
    guides: "/ai + [câu hỏi], và muốn xóa dữ liệu thì /ai clean",
    cd: 1,
  },
  ...
  onReply: async ({ api, message, event, onReply, args }) => {
    const prompt = args.join(" ");
    const uid = event.senderID;
    let question = args.join(" ");

    let { author, name } = onReply;
