...
this.onRun = async function ({ api, event, args }) {
    ...
    const duration = value.length && value.length.simpleText ? value.length.simpleText : 'Không có thông tin';
    
    return `|› ${index + 1}. ${value.title}\n|› 👤 Kênh: ${value.channelTitle}\n|› ⏱️ Thời lượng: ${duration}\n──────────────────`;
}).join('\n');

        return api.sendMessage(`📝 Có ${link.length} kết quả trùng với từ khóa tìm kiếm của bạn:\n──────────────────\n${msg}\n\n📌 Reply (phản hồi) STT để tải nhạc`, event.threadID, (error, info) => global.Seiko.onReply.push({
            type: 'reply',
