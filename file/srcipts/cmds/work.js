module.exports.config = {
    ...
    role: 0,
    author: "D-Jukie",
    info: "Làm việc để có tiền, có làm thì mới có ăn",
    Category: "Giải trí",
    cd: 5,
    envConfig: {
  ...
  module.exports.onReply = async ({ event, api, onReply, Currencies, getText }) => {
    const { threadID, messageID, senderID } = event;if(onReply.author!=senderID)return;
    let data = (await Currencies.getData(senderID)).data || {};
   var coin = Math.floor(Math.random() * (200000 - 100000 + 1)) + 100000;
  ...
  module.exports.onRun = async ({ event, api, onReply, Currencies, getText }) => {
