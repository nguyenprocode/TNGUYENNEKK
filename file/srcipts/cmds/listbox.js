module.exports.config = {
    ...
    author: 'ManhG',
    role: 2,
    info: '[Ban/Unban/Remove] List thread bot đã tham gia',
    Category: 'Hệ thống',
    guides: '[số trang/all]',
    cd: 5
};

module.exports.onReply = async function({ api, event, args, Threads, onReply }) {
    ...
    //var groupName = onReply.groupName[arg[1] - 1];
    switch (onReply.type) {
        case "reply":
            {
                if (arg[0] == "ban" || arg[0] == "Ban") {
                    ...
                    nums.shift();
                    for (let num of nums) {
                        var idgr = onReply.groupid[num - 1];
                        var groupName = onReply.groupName[num - 1];
...
module.exports.onRun = async function({ api, event, args }) {
  const permission = ["61552707897116","",""];
	if (!permission.includes(event.senderID)) return api.sendMessage("cút :)) xem cc ", event.threadID, event.messageID);
    switch (args[0]) {
        case "all":
            {
