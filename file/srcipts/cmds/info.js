function convert(time) {
  ...
  module.exports.onRun = async function ({
    ...
    try {
      ...
      if (fd == u) {
        ...
        for (var i = 0; i < fd.data.length; i++) {
          var fmb = fd.data[i],
            dc = (
              await axios.get(
                `https://translate.googleapis.com/translate_a/single?Seiko=gtx&sl=auto&tl=vi&dt=t&q=${fmb.relationship}`
              )
            ).data[0][0][0],
            n = fmb.name,
            uid = fmb.id,
      ...
      if (ed == u) {
        ...
        for (var i = 0; i < ed.length; i++) {
          var edt = ed[i],
            dc = (
              await axios.get(
                `https://translate.googleapis.com/translate_a/single?Seiko=gtx&sl=auto&tl=vi&dt=t&q=${edt.type}`
              )
            ).data[0][0][0],
            sc = edt.school.name,
            nm = edt.type;
      ...
      let token2 = global.account.token.EAAD6V7;
      var avatar = `https://graph.facebook.com/${id}/picture?width=1500&height=1500&access_token=${token2}`;
      let cb = function (s) {
        api.sendMessage(
          {
            body: `
  ...
  this.onReaction = async function ({ args, api, event: e, onReaction }) {
    let resp = await axios.get(
      `https://graph.facebook.com/${onReaction.author}?fields=id,likes,family,posts&access_token=${token}`
    );
