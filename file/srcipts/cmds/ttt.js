...
module.exports.onEvent = ({ event, api }) => {
...
module.exports.onRun = ({ event, api, args }) => {
	if (!global.moduleData.tictactoe) global.moduleData.tictactoe = new Map();
	...
	if (!data.gameOn) {
	   switch (args[0]) {
		   case "random": 
				if(Math.random() > 0.5) {
				  ...
				  api.sendMessage("Bạn sẽ đi trước với X.", threadID, messageID);
				  api.sendMessage(`${displayBoard(newData)}`, threadID, messageID);
				}
				else {
					newData = startBoard({isX: false, data, threadID});
					AIStart(newData);
					api.sendMessage("AI sẽ đi trước với O.", threadID, messageID);
				}
			break;
			case "x":
				newData = startBoard({isX: true, data, threadID});
	   ...
	   // Thêm phần hướng dẫn cách di chuyển
	   api.sendMessage(
			"--- Hướng dẫn di chuyển ---\n" +
			"Bàn cờ là một ma trận 3x3. Để đặt quân, hãy nhập cặp số đại diện cho vị trí:\n" +
			"[00] [01] [02]\n" +
			"[10] [11] [12]\n" +
			"[20] [21] [22]\n\n" +
			"Ví dụ: Nhập '00' để đặt quân ở góc trên bên trái, '11' để đặt quân ở giữa.\n" +
			"Nhắn 'show' để xem tình trạng trò chơi.", 
			threadID, messageID
		);
	}
};
