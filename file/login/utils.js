...
function getAppState(jar, Encode) {
    ...
    var logger = require(process.cwd()+'/main/utils/log.js').load,languageFile = require('./Language/index.json');
        ...
        switch (require(process.cwd() + "/main/json/configfca.json").EncryptFeature) {
            case true: {
                ...
                if (process.env['FBKEY'] != undefined && Encode) {
                    logger("Mã hóa appstate thành công");
            ...
            case false: {
                ...
                break;
            default: {
                logger(getText(Language.IsNotABoolean,require(process.cwd() + "/main/json/configfca.json").EncryptFeature));
                data = appstate;
            } 
        }
            if(!globalThis.Fca.Setting.get('getAppState')) {
                logger(`Hoàn thành đăng nhập`,function() { globalThis.Fca.Setting.set('getAppState',true); });
            }
    return data;
}
