  ...
  mqttClient.on('connect', () => {
    ...
    if (process.env.OnStatus === undefined) {
      ...
      const SettingMemoryManager = {
        ...
        interval: 60 * 1000,
        logLevel: 'warn',
        logFile: path.join(process.cwd(), 'main', 'db' ,'sqlite', 'memory.log'),
...
function LogUptime() {
  ...
  const filePath = join(process.cwd() + '/main/json/CountTime.json');
...
if (global.Fca.Require.FastConfig.AntiGetInfo.AntiGetThreadInfo) {
  setInterval(() => {
    try {
      ...
      if (!Data || typeof Data.keys !== 'function') return;
      ...
      for (let i of Arr) {
          ...
          if (hasData(i)) {
              let x = getData(i);
              if (x && typeof x.messageCount === 'number') {
