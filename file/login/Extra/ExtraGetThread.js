...
const log = require(process.cwd()+'/main/utils/log.js');
...
if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "json") {
    if (!fs.existsSync(process.cwd() + "/main/db/sqlite/Threads.json")) {
        fs.writeFileSync(process.cwd() + "/main/db/sqlite/Threads.json",JSON.stringify({}));
    }
}
    ...
    log.warn("Database_Type in FastConfigFca.json is not valid. Only default and json are valid");
    process.exit(0);
}
exports.createData = function(threadID,threadData) {
    if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "default") {
        try { 
            ...
            log(`Khởi tạo thành công data cho nhóm: ${String(threadID)}`, "[ LOGIN ]");
        } catch (e) {
            console.log(e);
            log.warn(`Khởi tạo thất bại data nhóm: ${String(threadID)}`, "[ LOGIN ]"); 
        }
    } else if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "json") {
        try {
            try {
                var data = require(process.cwd() + "/main/db/sqlite/Threads.json");
            } catch (e) {
                var data = {};
                fs.writeFileSync(process.cwd() + "/main/db/sqlite/Threads.json",JSON.stringify(data));
            }    
...
exports.updateData = function(threadID,threadData) {
    if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "default") {
        try { 
            Database(true).set(String(threadID),Object(threadData));
            log(`Cập nhật data cho nhóm: ${String(threadID)} thành công`, "[ LOGIN ]");
        }
        catch (e) {
            console.log(e);
            log.warn(`Cập nhật data cho nhóm: ${String(threadID)} thất bại`, "[ LOGIN ]"); 
        }
    }
    else if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "json") {
        try {
...
exports.deleteAll = function(data) {
    if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "default") {
        ...
        try {
            var data1 = require(process.cwd() + "/main/db/sqlite/Threads.json");
            for (let i of data) {
                delete data1[String(i)];
            }
            fs.writeFileSync(process.cwd() + "/main/db/sqlite/Threads.json",JSON.stringify(data1));
        }
        catch (e) {
            console.log(e);
        }
...
exports.alreadyUpdate = function(threadID) {
    if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "default") {
        var Time = Database(true).get(String(threadID)).TimeUpdate;
            ...
            log(`Nhóm ${String(threadID)} đã sẵn sàng để update data!`, "[ LOGIN ]");
                    return true;
                }
                else return false;
            }
