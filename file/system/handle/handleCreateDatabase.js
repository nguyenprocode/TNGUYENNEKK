    ...
    return async function ({ event }) {
        ...
        try {
            if (!allThreadID.includes(threadID) && event.isGroup) {
                ...
                if (!threadExists) {
                    const threadIn4 = await api.getThreadInfo(threadID);
                    const dataThread = {
                    ...
                    for (const singleData of threadIn4.userInfo) {
                        if (singleData.gender !== undefined) {
                            ...
                            if (!allUserID.includes(String(singleData.id))) {
                                const userExists = await Users.getData(singleData.id);
                                if (!userExists) {
            ...
            if (!allCurrenciesID.includes(senderID)) {
                const currencyExists = await Currencies.getData(senderID);
                if (!currencyExists) {
                    const setting4 = {
                        data: {}
                    };
